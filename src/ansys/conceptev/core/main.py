...
def create_new_project(client: httpx.Client, account_id: str, hpc_id: str, title: str,
    ...
    created_project = httpx.post(osm_url + '/project/create',
                                 headers={"Authorization": token}, json=project_data)
    ...
    created_design = httpx.post(osm_url + '/design/create',
                                headers={"Authorization": token}, json=design_data)
    ...
    user_details = httpx.post(osm_url + '/user/details',
    ...
    concept_data = {
        ...
        "design_instance_id": created_design.json()['designInstanceList'][0][
            'designInstanceId'],
        "drive_cycles_ids": [],
        "jobs_ids": [],
        "name": "Branch 1",
...
def get_account_ids(token: str) -> dict:
    ocm_url = os.environ["OCM_URL"]
    response = httpx.post(url=ocm_url + "/account/list",
                          headers={"authorization": token})
    if response.status_code != 200:
...
aero2 = {"name": "Second Aero Configuration",
...
motor_params = {"component_name": "e9",
                "component_file_type": "motor_lab_file"}

battery = {"capacity": 86400000,
...
if __name__ == '__main__':
    ...
    with get_http_client(token) as client:  # Create a client to talk to the api
        ...
        created_concept = create_new_project(client, account_id, hpc_id,
                                             f"New Project +{datetime.datetime.now()}")  # Create a new concept.
    ...
    with get_http_client(token,
                         concept_id) as client:  # get client with concept id embedded in
        ...
        configurations = get(client, '/configurations',
                             params={'config_type': 'aero'})  # read all aeros
        print(configurations)

        aero = get(client, "/configurations",
                   id=created_aero['id'])  # get a particular aero configuration
        print(aero)

        ### Create Components
        created_transmission = post(client, "/components",
                                    data=transmission)  # create transmission

        created_motor = post_file(client, "/components", motor_file_name,
                                  params=motor_params  # create motor from file
                                  )
        print(created_motor)
        client.timeout = 2000  # Needed as these calculations will take a long time.
        motor_loss_map = post(client, '/components:calculate_loss_map',data = {},
                              params={"component_id":created_motor['id']})  # get loss map from motor
        ...
        created_battery = post(client, "/components",
                               data=battery)  # create battery
        ...
        created_arch = post(client, "/architectures",
                            data=architecture)  # create arch
        ...
        ### Submit job
        concept = get(client, "/concepts", id=design_instance_id,
